export class ColorConverter {
	...
	convertColorToHex(colorName) {
		return this.colorMap[colorName.toLowerCase()] || colorName;
	}

	convertSVGColors(svgString) {
		return svgString.replace(/fill=['"]([^'"]+)['"]|stroke=['"]([^'"]+)['"]/g, (match, p1, p2) => {
			if (p1) {
				return `fill="${this.convertColorToHex(p1)}"`;
			}
			if (p2) {
				return `stroke="${this.convertColorToHex(p2)}"`;
			}
			return match;
		});
	}

	convertStyleToAttributes(svgString) {
		return svgString.replace(/style=['"]([^'"]+)['"]/g, (match, styleContent) => {
			const styleAttributes = styleContent.split(';').map(s => s.trim()).filter(s => s.length > 0);
			const svgAttributes = styleAttributes.map(attr => {
